...
echo "[ v3_req ]" > extfile.cnf
echo "extendedKeyUsage = clientAuth,serverAuth" >> extfile.cnf
# see http://wiki.cacert.org/FAQ/subjectAltName
# echo "subjectAltName = IP:$PRIVATE_IP" >> extfile.cnf
echo "subjectAltName = @alt_names" >> extfile.cnf
echo >> extfile.cnf
echo "[alt_names]" >> extfile.cnf
echo "IP.1 = $PRIVATE_IP" >> extfile.cnf
echo "IP.2 = $PUBLIC_IP" >> extfile.cnf
openssl x509 -req -days $DAYS -passin pass:$PASS -in server.csr -CA ca.pem -CAkey ca-key.pem -out server-cert.pem -extfile extfile.cnf -extensions v3_req
